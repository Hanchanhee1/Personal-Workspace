...
const FindPassword: React.FC = () => {
  ...
  const handleFindPassword = async (e: React.FormEvent) => {
    ...
    setError(null);
    setResult(null);
    // 실제 서비스에서는 이메일로 비밀번호 재설정 링크 발송 API 필요
    setTimeout(() => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 15, color: '#888', marginBottom: 0 }}>비밀번호 찾기</div>
        </div>
        {error && (
        ...
        <form onSubmit={handleFindPassword} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <input
            type="email"
